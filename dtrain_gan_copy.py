...
class TrainGAN():
    def __init__(self, opt, attributes, cuis_ingres, unseenAtt, unseenLabels, writter, seen_feats_mean, gen_type='FG'):
        ...
        self.cuisines = cuis_ingres[0].cuda()
        self.ingredients = cuis_ingres[1].cuda()

        self.branch1 = nn.Sequential(
            nn.Linear(self.opt.attSize, self.opt.attSize * 2),
            nn.LeakyReLU(0.2, True),
            nn.Linear(self.opt.attSize * 2, self.opt.attSize),
            nn.ReLU(),
        )
        ...
        # self.sim_MSE = nn.MSELoss()
        self.sim_MSE = nn.CrossEntropyLoss()
        ...
        # self.netD = model.MLP_3HL_CRITIC(self.opt)
        self.netG = dmodel_old.LatentDiffusion(self.opt)
        ...
        if self.opt.cuda and torch.cuda.is_available():
            ...
            self.branch1 = self.branch1.cuda()
            self.branch2 = self.branch2.cuda()
            self.branch3 = self.branch3.cuda()
        ...
        self.optimizerG = optim.AdamW(self.netG.parameters(), lr=self.opt.lr, betas=(self.opt.beta1, 0.999))

        # self.optimizerTranscoder = optim.Adam(self.Transcoder.parameters(), lr=self.opt.lr, betas=(self.opt.beta1, 0.999))

        self.num_clusters = 10

        # self.learnable_cuisine_mask = nn.Parameter(torch.randn(opt.batch_size,opt.attSize).double(), requires_grad=True).cuda()
        # self.learnable_ingredient_mask = nn.Parameter(torch.randn(opt.batch_size,opt.attSize).double(), requires_grad=True).cuda()

        self.learnable_cuisine_mask = nn.Parameter(torch.bernoulli(torch.rand(opt.batch_size, opt.attSize).double()),
                                                   requires_grad=True).cuda()
        self.learnable_ingredient_mask = nn.Parameter(torch.bernoulli(torch.rand(opt.batch_size, opt.attSize).double()),
                                                    requires_grad=True).cuda()
    ...
    def trident_mlp(self, attribute_vector):
        att1 = self.branch1(attribute_vector)
        att2 = attribute_vector
        # att2 = self.branch2(attribute_vector)
        att3 = self.branch3(attribute_vector)
    ...
    def load_checkpoint(self):
        ...
        net_G1 = self.opt.netG.replace('gen_best', 'gan_best')
        self.netG1.load_state_dict(torch.load(net_G1)['state_dict'])
    ...
    def save_checkpoint(self, state='latest'):
        ...
        torch.save({'state_dict': self.netG1.state_dict(), 'epoch': self.epoch}, f'{self.opt.outname}/gan_{state}.pth')
    ...
    def save_each_epoch_checkpoint(self, state='latest'):
        ...
        torch.save({'state_dict': self.netG1.state_dict(), 'epoch': self.epoch},
                   f'{self.opt.outname}/gan_{state}.pth')

    def generate_syn_feature(self, labels, attribute, embedding_gcnv=None, num=100, no_grad=True):
        """
        generates features
        inputs:
        ...
        if no_grad is True:
            with torch.no_grad():
                for i in range(nclass):
                    ...
                    input_att1, input_att2, input_att3 = self.trident_mlp(syn_att)

                    syn_noise.normal_(0, 1)
                    output2 = self.netG1(Variable(syn_noise), Variable(input_att2))

                    bs_t = syn_att.shape[0]

                    syn_attv1 = torch.cat([output2, input_att1], dim=1)
                    syn_attv3 = torch.cat([output2, input_att3], dim=1)
                    output1 = self.netG.sample([syn_attv1, syn_attv3], batch_size=bs_t)
                    output1 = output1[0].squeeze()

                    # syn_attv3 = torch.cat([output2, input_att3], dim=1)
                    # output3 = self.netG.sample(syn_attv3, batch_size=bs_t)
                    # output3 = output3[0].squeeze()
                    ...
                    for j in range(self.num_clusters):
                        indices = np.where(cluster_ids_x == j)[0]
                        top_indices = indices[np.argsort(-np.linalg.norm(output[indices] - cluster_centers[j], axis=1))][
                                      :50]
                        top_features.extend(top_indices)

                    # Make sure we have exactly 500 features
                    if len(top_features) < 500:
                        # If we don't have enough features, add more from each cluster until we have exactly 500
                        for j in range(self.num_clusters):
                            # Get the indices of the data points in this cluster
                            indices = torch.where(cluster_ids_x == j)[0]

                            # Add as many features as we need from this cluster to get up to 500 total features
                            while len(top_features) < 500 and len(indices) > 0:
                                if indices[0] >= 500:
                                    top_features.append(indices[0])
                                indices = indices[1:]

                            if len(top_features) >= 500:
                                break

                    output = output[top_features].cuda()

                    syn_feature.narrow(0, i * num, output.shape[0]).copy_(output.data.cpu())
                    syn_label.narrow(0, i * num, output.shape[0]).fill_(label)
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            for p in self.netG1.parameters():  # reset requires_grad
            ...
            input_att1, input_att2, input_att3 = self.trident_mlp(real_att)

            input_att = Variable(input_att2)
            ...
            for iter_d in range(self.opt.critic_iter):
                ...
                criticD_real = criticD_real.unsqueeze(0)
                ...
                fake = self.netG1(z_conc, label_conc)
                ...
                criticD_real2 = criticD_real2.unsqueeze(0)
            ...
            # ---------------------
            # separable semantic loss
            ##
            cuisine_sim = self.cosine_sim(input_att1.unsqueeze(1), self.cuisines.unsqueeze(0))
            cuisine_loss = self.sim_MSE(cuisine_sim, self.learnable_cuisine_mask)
            cuisine_loss.backward()

            ingredient_sim = self.cosine_sim(input_att3.unsqueeze(1), self.ingredients.unsqueeze(0))
            ingredient_loss = self.sim_MSE(ingredient_sim, self.learnable_ingredient_mask)
            ...
            input_attv_1 = fake1.data.detach()
            input_att1 = input_att1.data.detach()
            input_attv1 = torch.cat([input_attv_1, input_att1],dim=1).unsqueeze(-2)

            input_attv_3 = fake1.data.detach()
            input_att3 = input_att3.data.detach()
            input_attv3 = torch.cat([input_attv_3, input_att3], dim=1).unsqueeze(-2)

            loss1, loss_dic1 = self.netG(input_resv, [input_attv1, input_attv3])
            ...
            # input_att3 = input_att3.data.detach()
            # input_attv3 = torch.cat([input_attv_3, input_att3], dim=1).unsqueeze(-2)
            # loss3, loss_dic3 = self.netG(input_resv, input_attv3)
            #
            # loss3.backward()
            #
            # loss = loss1 + loss3
            ...
            self.tensorwrite.add_scalar("train_loss_df", loss, self.epoch*self.ntrain + i)
            self.tensorwrite.add_scalar("train_loss_gangen", errG.item(), self.epoch * self.ntrain + i)
            self.tensorwrite.add_scalar("train_loss_gandis", D_cost.item(), self.epoch * self.ntrain + i)
