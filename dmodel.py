...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def sample_all(self, num_samples, batch_size, y_dist, ddim=False):
        ...
        return x_gen, c_gen, y_gen

class DiffusionWrapper(torch.nn.Module):
    def __init__(self, opt):
        super().__init__()

        self.d_in = opt.resSize + opt.nclass_all
        # self.d_in = opt.resSize
        self.num_classes = opt.nclass_all
        self.is_y_cond = opt.is_y_cond
        self.rtdl_params = opt.rtdl_params
        self.dim_t = opt.dim_t
        self.dim_att = opt.resSize + opt.attSize

        self.diffusion_model = MLPDiffusion(self.d_in, self.num_classes, self.is_y_cond,
                                            self.rtdl_params, self.dim_t, self.dim_att)

    def forward(self, x, t, y=None):
        out = self.diffusion_model(x, t, y)
        return out
